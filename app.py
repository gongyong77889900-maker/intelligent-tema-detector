...
class MultiLotteryCoverageAnalyzer:
    ...
    def enhanced_extract_numbers(self, content, lottery_category='six_mark'):
        ...
        try:
                ...
                for pattern in colon_patterns:
                    ...
                    if match:
                        ...
                        if number_part:
                            ...
                            if all_possible_numbers:  # 如果成功提取到号码，直接返回
                                return list(set(all_possible_numbers))
            
            # 🆕 修复：处理多种分隔符格式 - 改进逻辑确保第一个号码不被跳过
            separators = [',', '，', ' ', ';', '；', '、', '/', '\\', '|']
            
            # 先尝试最常见的逗号分隔
            if ',' in content_str or '，' in content_str:
                # 使用正则表达式确保正确分割
                number_parts = re.split(r'[,，]', content_str)
                for part in number_parts:
                    # 🆕 修复：从每个部分提取所有数字，不跳过任何部分
            ...
            # 🆕 修复：处理连续数字格式（如123456）
            if re.match(r'^\d{2,}$', content_str.replace(' ', '')):
            ...
            # 🆕 修复：处理常见格式：3,4,5,6,15,16,17,18
            if re.match(r'^(\d{1,2},)*\d{1,2}$', content_str.replace(' ', '')):
                clean_content = content_str.replace(' ', '')
                new_numbers = [int(x.strip()) for x in clean_content.split(',') if x.strip().isdigit()]
                all_possible_numbers.extend(new_numbers)
                if all_possible_numbers:
                    return list(set(all_possible_numbers))
            ...
            all_possible_numbers = [num for num in all_possible_numbers if num in number_range]
            all_possible_numbers.sort()
